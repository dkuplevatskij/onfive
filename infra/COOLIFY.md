...
5. **Environment Variables** — заполни по `infra/.env.example`. Секреты генерируй:
   ...
   ```
   `ANON_KEY` и `SERVICE_ROLE_KEY` — это JWT-токены, подписанные `JWT_SECRET`.
   Сгенерируй их готовым скриптом (тем же `JWT_SECRET`, что в стеке):
   ```bash
   JWT_SECRET="<твой JWT_SECRET>" python3 infra/scripts/gen-keys.py
   ```
   Альтернатива — [генератор из доки Supabase](https://supabase.com/docs/guides/self-hosting/docker#generate-api-keys).
